...
QString NovelAIBase::getPunc(QString para, int pos)
{
    ...
    while (sent2_left > 0 && !isSentPunc(para.mid(sent_left - 1, 1)) )
        sent2_left--;
    punc = getTalkTone(para.mid(sent_left, sent_right - sent_left), para.mid(sent2_left, sent_right - sent2_left), tone, left1, left2);
 ...
 * @return       应该添加的标点
 */
QString NovelAIBase::getTalkTone(QString sent, QString sent2, int tone, QString left1, QString left2)
{
    Q_UNUSED(sent2);
    Q_UNUSED(left1);
    Q_UNUSED(left2);
...
QString NovelAIBase::getPunc(int pos)
{
    ...
    QString para = _text.mid(l_pos, r_pos - l_pos);
    return (getPunc(para, pos));
}
...
{
    _auto_punc_whitelists = "说道问吗哼呀呢么嘛啦呵吧啊哦哪呸滚";
    _shuo_blacklists = "[白|在|再有|接|一直|人|没|持|道|还|样|以|理|明|里|来|么|用|要|一|想|都|是|虽|听|听我|据|再|小|说|是|不|传|如|胡|话|乱|且|假|游|按|邪|述|数|陈|成|称|浅|图|谬|定|解|瞎|劝|妄|解|叙|絮|评|照|论|申|言|演|学|明|好|难|枉|接|实|者|上|一会儿?|被.*|听|不|无.*|让.*|怎么.*|说.*]说";
    ...
    _sent_split_puncs = "，。？！；~—…?!;\"“”,.";
    _sent_split = "\n\t，。？！；~—…?!;\"“”,.";
    _quote_no_colon = "点击倒真是成为其乃就能的作些称作之被有和及选择在会用不起么上下出入与和及跟叫并且可以要非来去离知何啥意一百千万亿为到拿以";
...
bool NovelAIBase::isCursorInQuote(QString text, int pos)
{
    ...
    // 搜索左右两边的最近的前后引号
    int ll_pos = text.lastIndexOf("“", pos);
    int lr_pos = text.lastIndexOf("”", pos);
