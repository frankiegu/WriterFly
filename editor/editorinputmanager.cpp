...
{
    EditorInputCharactor* charactor = new EditorInputCharactor(_edit, point, str, position, _edit->font(), delay, duration);
    ...
    f2.setForeground(QBrush(QColor(128,128,128, 0)));
...
{
    if (position > -1 && position < _edit->toPlainText().length())
    {
        QTextCursor cursor = _edit->textCursor();
        QTextCharFormat f = cursor.charFormat();
        QTextCharFormat f2 = f;
