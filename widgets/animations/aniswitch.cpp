...
void AniSwitch::paintEvent(QPaintEvent *e)
{
    ...
    int h = height(), w = width()*9/10;
    if (w > h*2) w = h * 2;
    if (w < h*2) h = w / 2;
    int r = (h+1)/3;
    int l = (width()-w), t = (height()-h)>>1; // 靠右对齐
    ...
    painter.fillPath(pathBack, QBrush(QColor(0x88, 0x88, 0x88+0x66*slider_pos/100)));

    // 画圆形滑块背景
    QPainterPath pathBack3;
    pathBack3.setFillRule(Qt::WindingFill);
    pathBack3.addEllipse(QRect(l+(w-r-r)*slider_pos/100-1, t+h/6-1, (r<<1)+2, (r<<1)+2));
