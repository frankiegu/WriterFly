...
void ImportPage::initLayout()
{
    ...
    QString trans = "QPushButton{border-width:1px;border-style:solid;border-color:rgb(128,128,128,80); background-color: transparent; padding:5px}";
    ...
    QLabel* roll_rx_label = new QLabel("卷名正则", this);
    QLineEdit* roll_rx_edit = new QLineEdit(roll_reg = us->getStr("import/roll_regexp", "(第.{1,5}卷) +([^\\n]+)\\n"));
    ...
    QLabel* chpt_rx_label = new QLabel("章名正则", this);
    QLineEdit* chpt_rx_edit = new QLineEdit(chpt_reg = us->getStr("import/chpt_regexp", "(第.{1,5}章) +([^\\n]+)\\n"));
...
void ImportPage::slotStartImport()
{
    ...
    QString dir_path = novel_path+"/dir.txt";       // 小说目录
	...
	QString total_content = readTextFile(file_path); // 导入的内容（全部）
