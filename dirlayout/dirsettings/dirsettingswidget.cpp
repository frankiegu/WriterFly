...
void DirSettingsWidget::initLayout()
{
	...
	// 显示章节时间
    QHBoxLayout* show_change_time_hlayout = new QHBoxLayout();
    MyLabel* show_change_time_label = new MyLabel("显示修改时间 [未开发]", this);
    show_change_time_switch = new AniSwitch(false, this);
    show_change_time_hlayout->addWidget(show_change_time_label);
    show_change_time_hlayout->addWidget(show_change_time_switch);
    show_change_time_label->setFont(font);
    show_change_time_label->setToolTip("在每一章节的下面显示修改时间 [暂时不支持]");
    show_change_time_switch->setMaximumWidth(width()/5);
    show_change_time_switch->setResponse();
    connect(show_change_time_label, &MyLabel::clicked, [=]{
    	show_change_time_switch->slotSwitch();
    	setVal("show_change_time", show_change_time_switch->isOn());
    });
    connect(show_change_time_switch, &AniSwitch::signalSwitch, [=]{
        setVal("show_change_time", show_change_time_switch->isOn());
    });
    ...
    main_vlayout->addLayout(show_change_time_hlayout);
...
void DirSettingsWidget::initData()
{
    ...
    show_change_time_switch->setOnOff(s->getBool("show_change_time", false));
