...
void NovelDirMData::readDir(QString novel_name)
{
    ...
    if (!novel_dir.exists())
    {
        QMessageBox::information(nullptr, QObject::tr("读取书籍失败"), QObject::tr("小说文件夹“%1”不存在").arg(book_path));
        return ;
    }
    // 判断小说文件是否存在
    ...
    if (!dir_file.exists())
    {
        QMessageBox::information(nullptr, QObject::tr("读取目录失败"), QObject::tr("目录文件“%1”不存在").arg(dir_file_path));
        return ;
    }
...
int NovelDirMData::addRoll(QString new_name)
{
    ...
    if (pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("创建新卷失败"), QObject::tr("找不到书籍末尾标记"));
        return 0;
    }
...
int NovelDirMData::addChapter(int list_index, QString new_name)
{
    ...
    if (name_pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("创建新章失败"), QObject::tr("找不到分卷起始标记"));
        return 0;
    }
    int pos = full_text.indexOf("</LIST>", name_pos);
    if (pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("创建新章失败"), QObject::tr("找不到分卷结尾标记"));
        return 0;
    }
...
bool NovelDirMData::tryChangeName(int index, QString new_name)
{
    ...
    if (item->isRoll())
    {
        ...
        if (pos != -1)
        {
            QMessageBox::information(nullptr, QObject::tr("修改卷名失败"), QObject::tr("分卷【%1】已存在！").arg(fn_new_name));
            gotoReedit();
            return false;
        }
        ...
        if (pos == -1)
        {
            QMessageBox::information(nullptr, QObject::tr("修改卷名失败"), QObject::tr("未找到分卷【%1】标记！").arg(old_name));
            return false;
        }
        ...
        if (end_pos <= 0)
        {
            QMessageBox::information(nullptr, QObject::tr("修改卷名失败"), QObject::tr("未找到分卷名字结束标记！"));
            return false;
        }
    ...
    else // 修改章节名字，需要改变文件名以及正在编辑的章节名字
    {
        ...
        if (pos != -1)
        {
            QMessageBox::information(nullptr, QObject::tr("修改章名失败"), QObject::tr("章节【%1】已存在！").arg(fn_new_name));
            gotoReedit();
            return false;
        }
        ...
        if (pos == -1)
        {
            QMessageBox::information(nullptr, QObject::tr("修改章名失败"), QObject::tr("未找到章节【%1】标记！").arg(old_name));
            return false;
        }

        int start_pos = pos+6;
        int end_pos = full_text.indexOf(QString(">"), pos)+1;
        if (end_pos <= 0)
        {
            QMessageBox::information(NULL, QObject::tr("修改章名失败"), QObject::tr("未找到章节名字结束标记！"));
            return false;
        }
...
int NovelDirMData::insertChapter(int index, QString new_name)
{
    ...
    {
        QMessageBox::information(NULL, QObject::tr("创建新卷失败"), QObject::tr("名称重复"));
        return 0;
    }
...
int NovelDirMData::insertRoll(int index, QString newName)
{
    ...
    {
        QMessageBox::information(nullptr, QObject::tr("创建新卷失败"), QObject::tr("名称重复"));
        return 0;
    }
...
QString NovelDirMData::deleteChapter(int list_index)
{
    ...
    if (left_pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("删除章节失败"), QObject::tr("找不到章节起始标记"));
        return "";
    }
    int right_pos = full_text.indexOf(QString("</CHPT>"), left_pos);
    if (right_pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("删除章节失败"), QObject::tr("找不到章节结尾标记"));
        return "";
    }
    right_pos += 7;
...
QStringList NovelDirMData::deleteRoll(int list_index)
{
    ...
    // 确认删除
    if (subs > 0 && QMessageBox::question(nullptr, QObject::tr("是否确认删除分卷【%1】？").arg(r_name), QObject::tr("包含章节数量：%1\n目前没有回收站功能，删除后将无法找回！").arg(subs), QMessageBox::Yes | QMessageBox::No, QMessageBox::No) == QMessageBox::No )
        return deleted_chapters;
    ...
    if (pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("删除分卷失败"), QObject::tr("找不到分卷名字标记"));
        return deleted_chapters;
    }
    int left_pos = full_text.lastIndexOf(QString("<ROLL>"), pos);
    if (left_pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("删除分卷失败"), QObject::tr("找不到分卷起始标记"));
        return deleted_chapters;
    }
    int right_pos = full_text.indexOf(QString("</ROLL>"), left_pos);
    if (right_pos == -1)
    {
        QMessageBox::information(nullptr, QObject::tr("删除分卷失败"), QObject::tr("找不到分卷结尾标记"));
        return deleted_chapters;
    }
    right_pos += 7;
    full_text = full_text.left(left_pos)+full_text.right(full_text.length()-right_pos);
